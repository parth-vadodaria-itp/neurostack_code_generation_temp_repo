...
- **Square**: Calculate the square of a number using custom loop-based logic (no `**` operator)
...
result = calc.square(4)          # 16 (using custom loop logic)
result = calc.square_root(25)    # 5.0
```

## Custom Square Implementation

The `square()` method uses a custom loop-based multiplication algorithm instead of the `**` operator:
- Handles integers and floating-point numbers
- Uses iterative addition for multiplication
- Correctly handles negative numbers (square is always positive)
